//
//  LTCDataManager.swift
//  NodaWallet
//
//  Created by macOsx on 24/06/20.
//  .
//
...
class LTCDataManager {
    ...
    class func getTransactionHash(parameter: [String: Any], completionHandler: @escaping (Bool, [String], [String: Any],  Data?, Error?) -> Swift.Void) -> () {
           ...
           Alamofire.request(Constants.urlConfig.LTC_CREATE_TX_NEW, method: .post, parameters: param,encoding: JSONEncoding.default, headers: header).responseJSON {
                  response in
    ...
    class func sendTransactionHash(parameter: [String: Any], completionHandler: @escaping (Bool, String, Data?, Error?) -> Swift.Void) -> () {
          ...
          let header = ["Content-Type": "application/json"]
          Alamofire.request(Constants.urlConfig.LTC_SEND_TX, method: .post, parameters: param, encoding: JSONEncoding.default, headers: header).responseJSON {
              response in
              switch response.result {
              case .success:
    ...
    class func getLtcTxDetails(hash: String, completionHandler: @escaping (Bool, String, Data?, Error?) -> Swift.Void) -> () {
        ...
        showAppLoader()
        let urlString = Constants.urlConfig.GET_LTC_TX_DETAIL_API + hash
